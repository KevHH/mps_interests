...
class Gifts {
  ...
  parse(lines) {
    ...
    for(line of lines) {
      ...
      gifts.push({"text": line.text, "donor": donor, "donor_address": donor_address, "description": description ,"amount": amount, "donor_status": donor_status})
    }
  ...
  addAll() {
    this.gifts.forEach(gift => this.add(gift))
  }

  add(donation) {
    let hash = md5(this.name + donation.text)

    let sql = "INSERT IGNORE INTO gifts(mp_id, mp_name, text, donor, donor_address, description, amount, donor_status, hash) VALUES(?,?,?,?,?,?,?,?,?)"
